...
const SalesHistory = ({ sales, downloadBill }) => {
    return (
        <div className="bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden animate-in fade-in duration-500">
            <div className="p-6 border-b border-slate-50 font-black text-slate-800 bg-slate-50/50 flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Digital Archive</span>
                </div>
            </div>
                    ...
                    <tbody className="divide-y divide-slate-50 font-medium">
                        {sales.map(s => (
                            <tr key={s._id} className="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <td className="px-6 py-4">
                                    <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-md ${s.paymentMethod === 'Credit (Udhaar)' ? 'bg-rose-50 text-rose-500' : 'bg-indigo-50 text-indigo-500'}`}>
                                        {s.paymentMethod}
                                    </span>
                                </td>
                                <td className="px-6 py-4 text-right font-black text-slate-800 text-lg">₹{s.total.toLocaleString('en-IN')}</td>
                                <td className="px-6 py-4 text-right">
                                    <button
                                        onClick={() => downloadBill(s)}
                                        className="bg-indigo-50 text-indigo-600 p-2 rounded-xl hover:bg-indigo-600 hover:text-white transition-all shadow-sm flex items-center gap-2 text-xs font-bold float-right"
                                    >
                                        <FileText size={16} /> <span className="hidden sm:inline">Download Bill</span>
                                    </button>
                                </td>
                            </tr>
                        ))}
