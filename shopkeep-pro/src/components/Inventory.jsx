...
import { Search, Edit3, Trash2, Calendar, AlertCircle, XCircle } from 'lucide-react';
                ...
                <div className="flex gap-1 bg-slate-100 p-1.5 rounded-2xl shrink-0">
                    {['all', 'low', 'out', 'expiring'].map(f => (
                        <button
                            ...
                            className={`px-4 py-2 rounded-xl text-xs font-black capitalize transition-all ${inventoryFilter === f ? 'bg-white shadow-md text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                                        ...
                                        <div className="flex flex-col gap-2">
                                            ...
                                            <div className="flex gap-1 flex-wrap">
                                                {outOfStock ? (
                                                    ...
                                                    <span className={`px-2 py-0.5 rounded-lg text-[9px] font-black flex items-center gap-1 ${item.stock <= (item.minStock || 5) ? 'bg-amber-100 text-amber-600' : 'bg-slate-100 text-slate-600'}`}>
                                                        {item.stock} UNITS
                                                    </span>
                                                )}

                                                {expired && (
                                                    <span className="bg-rose-600 text-white px-2 py-0.5 rounded-lg text-[9px] font-black flex items-center gap-1 shadow-sm">
                                            ...
                                            {item.expiryDate && (
                                                <span className={`text-[9px] font-bold flex items-center gap-1 ${expired ? 'text-rose-500' : 'text-slate-400'}`}>
                                    ...
                                    {isAdmin && (
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button onClick={() => { setEditProduct(item); setIsAddingProduct(true); }} className="p-2 hover:bg-indigo-50 text-indigo-500 rounded-xl transition-colors"><Edit3 size={16} /></button>
                                                <button onClick={() => deleteProduct(item._id)} className="p-2 hover:bg-rose-50 text-rose-500 rounded-xl transition-colors"><Trash2 size={16} /></button>
                                            </div>
                                        </td>
                                    )}
                                </tr>
                            ...
                            <tr>
                                <td colSpan={isAdmin ? 5 : 4} className="px-6 py-20 text-center">
                                    <div className="flex flex-col items-center justify-center opacity-40">
                                        ...
                                        <p className="text-slate-400 text-[10px] font-bold mt-1">Try adjusting your filters or search term.</p>
                                    </div>
                                </td>
                            </tr>
                        )}
